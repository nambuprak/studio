...
from server.services.embedding_service import embed_documents_for_tutor
...
def _update_status(status_dict: Dict[str, Any], message: str, processed_files_count: int = None, total_files_estimate: int = None, error: Optional[str] = None):
    if status_dict:
        if message:
...
def _process_files_in_directory(
    ...
    # Trim spaces and ensure file types start with a dot. Handle empty strings.
    file_types_to_include = [ft.strip() for ft in file_types_str.split(',') if ft.strip()]
    file_types_to_include = [ft if ft.startswith('.') else '.' + ft for ft in file_types_to_include if ft] # Ensure dot prefix

    # Trim spaces from excluded folder names. Handle empty strings.
    ...
    for root, dirs, files in os.walk(directory_path, topdown=True):
        ...
        dirs[:] = [] # Clear dirs, then add back only those not excluded

        for d_name in original_dirs: 
            dir_path_full = os.path.join(root, d_name)
            # For .gitignore matching, path should be relative to the directory_path (where .gitignore is located)
            dir_path_relative_to_walk_root = os.path.relpath(dir_path_full, directory_path)

            if d_name in folders_to_exclude_set:
                # print(f"Excluding by user rule (folder): {dir_path_full}")
                continue 
            
            # Exclude hidden directories (like .git, .vscode, etc.) by default
            if d_name.startswith('.'): 
                # print(f"Excluding hidden folder: {dir_path_full}")
                continue
            
            if gitignore_spec and gitignore_spec.match_file(dir_path_relative_to_walk_root + '/'): # Add trailing slash for dirs
                # print(f"Excluding by .gitignore (folder): {dir_path_full}")
                continue
        ...
        for file_name in files:
            ...
            if gitignore_spec and gitignore_spec.match_file(file_path_relative_to_walk_root):
                # print(f"Excluding by .gitignore (file): {file_path_full}")
                continue
            
            # Exclude hidden files unless explicitly included by a file_type
            if file_name.startswith('.') and not any(file_name.endswith(ft) for ft in file_types_to_include):
                # print(f"Excluding hidden file: {file_path_full}")
                continue

            if not file_types_to_include: # If no types specified, include all non-excluded files
                discovered_file_paths.append(file_path_full)
                current_file_count += 1
                _update_status(status_dict, progress_detail=f"Discovered file ({current_file_count}): {os.path.basename(file_path_full)}")
            else:
                _, ext = os.path.splitext(file_name)
                if ext.lower().strip() in file_types_to_include: # Compare lowercased and stripped extension
                    discovered_file_paths.append(file_path_full)
                    current_file_count += 1
                    _update_status(status_dict, progress_detail=f"Discovered file ({current_file_count}): {os.path.basename(file_path_full)}")
...
def process_repository_content(
    ...
    if input_type == 'folder':
        ...
        temp_clone_dir = f"temp_repo_clone_{tutor_id}" # Unique temp dir per tutor
        ...
        try:
            ...
            repo = git.Repo.clone_from(source_location, temp_clone_dir, depth=1) # Shallow clone for speed
            ...
            # For cloned repos, .gitignore is inherently handled by Git. We apply user's exclude_folders_str and file_types_str.
            ...
            raise Exception(f"Failed to clone repository: {error_details}") # Re-raise to be caught by _perform_long_repository_processing
        except Exception as e_process: # Catch other errors during cloning/setup
            ...
            if os.path.exists(temp_clone_dir):
                ...
                try:
                    # Attempt to fix permission errors on Windows for .git folder
                    if os.name == 'nt':
    ...
    if embed_repo_flag and discovered_file_paths:
        ...
        try:
            embed_documents_for_tutor(
                ...
                status_dict=status_dict # Pass the main status_dict for updates
            )
            processing_message += " Embedding process initiated." 
            # embedding_service will update status_dict with more details
            ...
            # Decide if this should re-raise or just log and continue
